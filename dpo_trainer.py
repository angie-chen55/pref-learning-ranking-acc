...
class DPOExperimentalTrainer(Trainer):
    r"""
    Initialize DPOExperimentalTrainer.
    ...
    def __init__(
        ...
        beta: float = 0.1,
        gamma: float = 0.1,  # Used only for the unlikelihood objective
        alpha: float = 0.1,  # Used only for the alpha_scaling objective
        ...
        self.requires_ref_model = self.loss_type not in [
            "no_sft_ratio",
            "no_sft_ratio_w_unlikedlihood",
            "ipo_nsr",
        ]
        ...
        if self.ref_model is None:
            if self.requires_ref_model and not (
                self.is_peft_model or self.precompute_ref_log_probs
            ):
                raise ValueError(
    ...
    def dpo_loss(
        ...
        policy_chosen_log_one_minus_probs: torch.FloatTensor = None,
        policy_rejected_log_one_minus_probs: torch.FloatTensor = None,
        ...
        Args:
            ...
            reference_chosen_logps (Optional): Log probabilities of the reference model for the chosen responses. Shape: (batch_size,)
            reference_rejected_logps (Optional): Log probabilities of the reference model for the rejected responses. Shape: (batch_size,)
            policy_chosen_log_one_minus_probs: Log(1 - probability) of the policy model for the chosen responses. Shape: (batch_size, )
            policy_rejected_log_one_minus_probs: Log(1 - probability) of the policy model for the rejected responses. Shape: (batch_size,)
        ...
        elif not self.requires_ref_model:
            ref_logratios = None
        else:
        ...
        if self.loss_type == "sigmoid":
        ...
        elif self.loss_type == "alpha_scaling":
            losses = -F.logsigmoid(
                self.beta * (pi_logratios - self.alpha * ref_logratios)
            )
        elif self.loss_type == "no_sft_ratio":
            losses = -F.logsigmoid(self.beta * pi_logratios)
        elif self.loss_type == "no_sft_ratio_w_unlikelihood":
            losses = -F.logsigmoid(
                self.beta * (policy_chosen_logps + policy_rejected_log_one_minus_probs)
            )
        elif self.loss_type == "unlikelihood":
            losses = -F.logsigmoid(
                self.beta
                * (
                    policy_chosen_logps
                    - reference_chosen_logps
                    + policy_rejected_log_one_minus_probs
                    - reference_rejected_logps
                )
            )
        ...
        elif self.loss_type == "ipo_nsr":
            losses = (pi_logratios - 1 / (2 * self.beta)) ** 2
            ...
            raise ValueError(
                f"Unknown loss type: {self.loss_type}. Should be one of ['sigmoid', 'hinge', 'ipo', 'kto_pair', 'unlikelihood']"
            )

        if self.requires_ref_model:
            chosen_rewards = (
                self.beta
                * (
    ...
    def get_batch_logps(
        ...
        label_pad_token_id: int = -100,
        is_encoder_decoder: bool = False,
        also_return_one_minus_logprobs: bool = False,
        ...
        Args:
            ...
            label_pad_token_id: The label pad token id.
            is_encoder_decoder: Whether the model is an encoder-decoder model.
            also_return_one_minus_logprobs: Whether to also return log(1-p).

        Returns:
            If also_return_one_minus_logprobs == False: A tensor of shape (batch_size,) containing the average/sum log probabilities of the given labels under the given logits.
            Otherwise, a tuple of (average/sum log probs,  sum log(1-prob))
        """
        ...
        log_one_minus_probs = torch.log(1.0 - torch.exp(per_token_logps))

        if average_log_prob:
            output = (per_token_logps * loss_mask).sum(-1) / loss_mask.sum(-1)
        else:
            output = (per_token_logps * loss_mask).sum(-1)
        if also_return_one_minus_logprobs:
            output = (output, (log_one_minus_probs * loss_mask).sum(-1))
        return output
    ...
    def concatenated_forward(
        ...
        batch_outputs = self.get_batch_logps(
            all_logits,
            concatenated_batch["concatenated_labels"],
            average_log_prob=self.loss_type == "ipo",
            is_encoder_decoder=self.is_encoder_decoder,
            label_pad_token_id=self.label_pad_token_id,
            also_return_one_minus_logprobs=self.loss_type
            in ["unlikelihood", "no_sft_ratio_w_unlikelihood"],
        )
        if self.loss_type in ["unlikelihood", "no_sft_ratio_w_unlikelihood"]:
            all_logps, log_one_minus_ps = batch_outputs
            chosen_log_one_minus_ps = log_one_minus_ps[:len_chosen]
            rejected_log_one_minus_ps = log_one_minus_ps[len_chosen:]
        else:
            all_logps = batch_outputs
            chosen_log_one_minus_ps = None
            rejected_log_one_minus_ps = None
        ...
        return (
            ...
            chosen_logits,
            rejected_logits,
            chosen_log_one_minus_ps,
            rejected_log_one_minus_ps,
        )
    ...
    def get_batch_loss_metrics(
        ...
        (
            ...
            policy_chosen_log_one_minus_probs,
            policy_rejected_log_one_minus_probs,
        ...
        if "reference_chosen_logps" in batch and "reference_rejected_logps" in batch:
            ...
            with torch.no_grad():
                if self.ref_model is None and self.requires_ref_model:
                    with self.null_ref_context():
                        (
                            reference_chosen_logps,
                            reference_rejected_logps,
                ...
                elif self.ref_model is None and not self.requires_ref_model:
        ...
        policy_accuracies = (policy_chosen_logps > policy_rejected_logps).float()

        prefix = "eval_" if train_eval == "eval" else ""
        metrics[f"{prefix}rewards/chosen"] = chosen_rewards.mean().cpu()
        metrics[f"{prefix}rewards/rejected"] = rejected_rewards.mean().cpu()
        metrics[f"{prefix}rewards/accuracies"] = reward_accuracies.mean().cpu()
        metrics[f"{prefix}policy/accuracies"] = policy_accuracies.mean().cpu()
        margins = chosen_rewards - rejected_rewards
        metrics[f"{prefix}rewards/margins"] = margins.mean().cpu()
    ...
    def get_batch_samples(
        ...
        with generate_context_manager():
            policy_output = model.generate(**gen_kwargs)
            ...
            if "reference_output" in batch:
                ...
                if self.ref_model is None and self.requires_ref_model:
                    with self.null_ref_context():
                        reference_output = self.model.generate(**gen_kwargs)
                elif self.ref_model is None and not self.requires_ref_model:
                    reference_output = None
                else:
                    reference_output = self.ref_model.generate(**gen_kwargs)

        policy_output_decoded = self.tokenizer.batch_decode(
            pad_to_length(
                policy_output.sequences, self.max_length, self.tokenizer.pad_token_id
            ),
            skip_special_tokens=True,
        )
        policy_output_scores = torch.stack(policy_output.scores).transpose(0, 1)
        policy_output_probs = torch.max(
            F.softmax(policy_output_scores, dim=-1), dim=-1
        ).values
        # must first find length of each output sequence, mask, and then sum
        seq_lens = []
        for seq_output in policy_output.sequences:
            is_eos = (seq_output == self.tokenizer.eos_token_id).nonzero()
            if is_eos.numel() <= 1:  # first EOS is the BOS token
                seq_lens.append(seq_output.numel())
            else:
                seq_lens.append(is_eos[1][0].item())
        seq_lens = torch.tensor(seq_lens)
        max_seq_len = policy_output_probs.shape[1]
        mask = (
            (
                torch.arange(max_seq_len).expand(len(seq_lens), max_seq_len)
                < seq_lens.unsqueeze(1)
            )
            .long()
            .cuda()
        )
        policy_output_logprobs = torch.sum(torch.log(policy_output_probs) * mask, dim=1)
        if not self.requires_ref_model:
            return (policy_output_decoded, policy_output_logprobs)

        reference_output_decoded = self.tokenizer.batch_decode(
            pad_to_length(
                reference_output.sequences, self.max_length, self.tokenizer.pad_token_id
            ),
            skip_special_tokens=True,
        )
        reference_output_scores = torch.stack(reference_output.scores).transpose(0, 1)
        reference_output_probs = torch.max(
            F.softmax(reference_output_scores, dim=-1), dim=-1
        ).values
        # must first find length of each output sequence, mask, and then sum
        seq_lens = []
        for seq_output in reference_output.sequences:
            is_eos = (seq_output == self.tokenizer.eos_token_id).nonzero()
            if is_eos.numel() <= 1:  # first EOS is the BOS token
                seq_lens.append(seq_output.numel())
            else:
                seq_lens.append(is_eos[1][0].item())
        seq_lens = torch.tensor(seq_lens)
        max_seq_len = reference_output_probs.shape[1]
        mask = (
            (
                torch.arange(max_seq_len).expand(len(seq_lens), max_seq_len)
                < seq_lens.unsqueeze(1)
        ...
        reference_output_logprobs = torch.sum(
            torch.log(reference_output_probs) * mask, dim=1
        )

        return (
            policy_output_decoded,
            policy_output_logprobs,
            reference_output_decoded,
            reference_output_logprobs,
        )
    ...
    def evaluation_loop(
        ...
        if self.generate_during_eval:
            ...
            if self.requires_ref_model:
                ...
                self.log(
                    {
                        "game_log": wandb.Table(
                            ...
                            rows=[
                                ...
                                for prompt, pol, pol_logprobs, ref, ref_logprobs in zip(
                                    random_batch["prompt"],
                                    policy_output_decoded,
                                    policy_output_logprobs,
                                    ref_output_decoded,
                ...
                generate_during_eval_metrics = {
                    "policy_generated_log_probs": policy_output_logprobs.mean(),
                    "reference_generated_log_probs": ref_output_logprobs.mean(),
                }
            else:
                (
                    policy_output_decoded,
                ...
                self.log(
                    {
                        "game_log": wandb.Table(
                            ...
                            rows=[
                                ...
                                for prompt, pol, pol_logprobs in zip(
                                    random_batch["prompt"],
                                    policy_output_decoded,
                                    policy_output_logprobs,
                                )
            ...
            self.store_metrics(generate_during_eval_metrics, train_eval="eval")
            self.state.log_history.pop()

        # Also compute weight norm
        model_weights_l2_norm = float(
            torch.FloatTensor(
                [
                    torch.sum(params.data**2)
        ...
        self.store_metrics(
            {"model_weights_l2_norm": model_weights_l2_norm}, train_eval="eval"
        )
